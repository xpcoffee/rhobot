...
function buildGetUserCommand(steamApiKey: string) {
  async function resolveVanityURL({ vanityName }) {
    const { response } = await fetch(
      `http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=${steamApiKey}&vanityurl=${vanityName}`
    ).then((res) => res.json());
  ...
  return {
    run: (message, parameters) => {
      ...
      if (userId) {
        ...
        summaryPromise = resolveVanityURL({ vanityName: vanity }).then(getPlayerSummary);
      }
...
function parseGetUserParams(parameters: string[]) {
  ...
  if (!result.userId && !result.vanity) {
    result.errors.push(
      "Enter either your 17-digit steam user ID using `--user-id` or your vanity URL name using `--vanity`"
    );
  }
