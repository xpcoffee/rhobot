...
export function buildCommand({
  ...
  if (!(ddbTable && ddbRegion)) {
    console.error(
      "[ERROR] Unable to create event command. Both DynamoDB table name and region are required. " +
        "Please check your app-config."
    );
    return undefined;
  }
  ...
  return {
    run: (message, parameters) => {
      ...
      message.channel.send(Event.formatLoading()).then((eventMessage) => {
        ...
        return createEvent(dao, channelId, eventMessage, event).catch((reason) =>
...
class EventDao {
  ...
  updateEvent(channelId, event: Event): Promise<string> {
    ...
    return this.ddb.put(channelId, EventDao.DATABASE_ITEM_TYPE, attributes).then(() => event.id);
  }
  ...
  deleteEvent(channelId: string, id: string): Promise<void> {
    return this.ddb.delete(channelId, EventDao.DATABASE_ITEM_TYPE, id).then(() => {
      /* do nothing */
    });
  }
  ...
  static eventToAttributes({
    ...
    created,
    maxParticipants,
  }: Event): AWS.DynamoDB.AttributeMap {
    const attributes = {
      uuid: { S: id },
      Title: { S: title },
...
class Event {
  ...
  static formatDeleted() {
    const embed = new MessageEmbed().setTitle("Event: 🗑️").setFooter("This event has been deleted.");
    return embed;
  }
}
